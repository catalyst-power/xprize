/**
 * DeliveryReceipt — async server component.
 *
 ...
 * The returned values are the signed asserted values — never derived or recomputed.
 */
  ...
  return (
    <section className="rounded-xl border border-zinc-800 bg-zinc-900/50 p-5">
      ...
      <p className="text-xs text-zinc-500">
        Receipt pending — delivery receipt has not been signed yet.
      </p>
    </section>
  );
}
...
export async function DeliveryReceipt(props: Readonly<{ correlationId: string }>) {
  ...
  const commodity = p.commodity ?? lot.commodity ?? '—';
  ...
  return (
    <section className="rounded-xl border border-zinc-700 bg-zinc-900 p-5 space-y-4">
      ...
      {/* Body — signed asserted values only; never recomputed */}
      ...
      {/* Provenance line — cryptographic anchor, NOT a physical-world truth claim */}
      <div className="border-t border-zinc-800 pt-3 space-y-1.5">
        ...
        <p className="text-[10px] text-zinc-600 leading-relaxed break-all">
          ...
          <span className="font-mono text-zinc-500">{attestationCid ?? '—'}</span>
          {priorCid !== null && (
            <>
              {' '}
              · chained to declared{' '}
        ...
        <p className="text-[10px] text-zinc-700 italic">
          Provenance not verdict — shows what was signed and by whom; not a claim of
